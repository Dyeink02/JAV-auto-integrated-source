...
* **packaging:** 新增 `desktop:dist:lite-exe` 直开式 EXE 打包链路，替代 `1.1.19` 的 Web 安装器路线
* **desktop:** 压缩首页背景资源，将标准便携版体积从 `87.17 MB` 降到 `84.34 MB`
* **docs:** 补充 `1.1.21` 直开轻量 EXE 说明

## [1.1.20] (2026-04-01)

### Bug Fixes

* **desktop:** 修复便携版按钮无响应问题，提前注册 IPC 并为渲染层增加初始化重试

### Features

* **desktop:** 为输出目录相关按钮补充可见日志反馈
* **test:** 新增打包后桌面 UI 烟测脚本 `scripts/smoke-desktop-ui.js`
* **docs:** 补充 `1.1.20` 按钮修复与便携版测试说明

## [1.1.19] (2026-04-01)

### Features

* **packaging:** 新增轻量安装版脚本 `desktop:dist:lite`，主推 50MB 以下的 Web 安装器分发
* **docs:** 补充 `1.1.19` 轻量安装版说明

## [1.1.18] (2026-04-01)

### Features

* **desktop:** 删除首屏 `Vibrant Refresh` 和 `4K Style Backdrop` 展示块，首屏结构回调到更接近 `1.1.16` 的紧凑布局
* **docs:** 补充 `1.1.18` 首页收缩版说明

## [1.1.17] (2026-03-31)

### Features

* **desktop:** 焕新桌面布局与状态面板，补充版本历史、背景来源和更年轻的视觉风格
* **docs:** 新增 `1.1.17` 版本说明与测试记录，便于后续滚动迭代
* **packaging:** 新增 `nsis-web` 分发方案，支持更小体积的 Windows 安装器

### Bug Fixes

* **crawler:** 修复 `limit` 模式下详情页会在对账/补队列阶段超量入队的问题

## [1.1.14] (2026-03-21)

### Features

* 提速优先包：修正详情页真实并发，页面与磁力改为 HTTP 优先 / Cloudflare 兜底链路
* 输出改为内存合并 + 批量落盘，减少大任务全量重写带来的 IO 开销
* 修复尾页分页缺口误判，并增强“未抓取番号 / 分页缺口”动态状态刷新

## [1.1.13] (2026-03-21)

### Features

* **status:** 状态区新增“正在执行 / 未抓取番号 / 分页缺口”展示，客户可直接查看当前执行项与缺失项

### Bug Fixes

* **paging:** 页锁定校验达到最大重抓次数后不再直接停任务，改为记录分页缺口、继续后续抓取并在任务结束后补查

## [1.1.12] (2026-03-21)

### Features

* **desktop:** 新增“打开磁力链接文档”入口，输出目录旁可直接打开 `magnet-links.txt`

### Bug Fixes

* **paging:** 当用户填写抓取数量时，按页执行严格锁页校验；非尾页未达到每页目标条数时禁止继续翻页
* **stability:** 优化大任务索引页并集校验、详情分批入队与状态落盘节流，减少卡顿与漏抓风险
