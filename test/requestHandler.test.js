...
describe('RequestHandler', () => {
  ...
  it('falls back to mirror page domains when the primary page domain is blocked', async () => {
    ...
    assert.ok(result);
    assert.ok(calledUrls[0].includes('www.javbus.com/star/ws2'));
    assert.ok(calledUrls.some((url) => url.includes('www.busjav.cyou/star/ws2')));
  ...
  it('limits page origin candidates to the healthiest top four entries', () => {
    ...
    assert.ok(origins.includes('https://www.javbus.com'));
  });
